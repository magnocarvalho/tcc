...
export class MapAdvancedComponent extends UpgradableComponent
  ...
  public ngAfterViewInit() {
    ...
    const getData = this.isWorldLevel
      ? this.mapService.getCountriesPopulation()
      : this.mapService.getCountriesPopulation();
    getData.pipe(takeWhile(() => this.alive)).subscribe((data) => {
      this.loadData(data);
    });
  ...
  private createMarkersArray(data) {
    ...
    data.map((item) => {
      ...
      marker.addListener("mouseover", (event) =>
        this.drawInfoWindow(item, event, marker)
      );
      marker.addListener("mouseout", (event) =>
        this.removeInfoWindow(item, event)
      );
      if (this.gMap.getZoom() <= switchZoomValue) {
        marker.addListener("click", (event) =>
          this.zoomingToPoint(item, event)
        );
      }
  ...
  private drawInfoWindow(item, event, marker): void {
    ...
    const mouseEvent = Object.values(event).find(
      (value) => value instanceof MouseEvent
    ) as Event;
    ...
    infowindow.style[position.left ? "left" : "right"] = `${iconSize +
      infoWindowSize.paddingX}px`;
  ...
  private calculateInfoPosition(marker): { top: boolean; left: boolean } {
    ...
    const worldCoordinate = this.gMap
      .getProjection()
    ...
    const position = {
      ...
      left:
        markerOffset.x + infoWindowSize.width + infoWindowSize.paddingX * 2 <
        mapOffset.x
    };
    return position;
  }
