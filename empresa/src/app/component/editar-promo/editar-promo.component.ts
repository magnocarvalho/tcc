...
export class EditarPromoComponent implements OnInit {
  ...
  uid: String;
  descont: Number = 0;
  constructor(private formBuilder: FormBuilder, public api: ApiService, public rota: Router, public store: PromoData) {
    api.user.subscribe(res => {
      this.uid = res.uid
    });
    this.form = this.formBuilder.group({
      title: [store.storage.title, [Validators.required, Validators.minLength(2), Validators.maxLength(100)]],
      about: [store.storage.about, [Validators.required, Validators.minLength(2)]],
      initPrice: [store.storage.initPrice, [Validators.required, Validators.min(1)]],
      endPrice: [store.storage.endPrice, [Validators.required, Validators.min(1)]],
      ...
      descont: [{ value: store.storage.descont, disabled: true }, [Validators.required]]
    }, {
